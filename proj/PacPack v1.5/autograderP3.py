...
"""
Autograder for Phase 3 of PacPack
"""
...
baselineScores = [816, 696, 867, 555, 738, 846, 933, 925, 625, 900]
...
class CaptureRules:
  ...
  def agentCrash(self, game, agentIndex):
    if agentIndex % 2 == 0:
      print >>sys.stderr, "Pacman with index {} crashed".format(agentIndex)
    else:
      print >>sys.stderr, "Ghost with index {} crashed".format(agentIndex)
...
def runGames(layouts, agents, display, length, numGames, record, numTraining, pacmanTeamName, ghostTeamName, muteAgents=False, catchExceptions=True ):
